class BinHeap():
	...
	#@start-editable@
        for i in range(0,(len(alist)-2)//2):
            #if conditions are not satisfied return false, else True
            
            #left child greater than parent
            if(alist[2*i]>alist[i]):
                return False

            #right child is lesser than parent
            if(alist[2*i+1]>alist[i]):
                return False
